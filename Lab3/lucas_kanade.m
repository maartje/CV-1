function flow = lucas_kanade(im1, im2, show_figures)
    ...
    % divides image in regions and
    % calculates the flow in x and y direction
    % for these regions by solving system of linear equation
    ...
    % stack flow components in 3D matrix
    flow = flowX;
    flow(:,:,2) = flowY;

    % show image with arrows indicating the flow
    % arrows are scaled to nicely fit within the image bounderies
    if show_figures
        figure;
        imshow(im1);
        hold on;
